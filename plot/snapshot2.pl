...
for($ii =0; $ii< @comp; $ii++)
{
   if($ii==0) {
       `psbasemap -JX$SCALE -R$RSCALE -K $PORT  -X$xshift -Y$yshift $BSCALE>$PS_file`;
        &PSTEXT($XMIN+($XMAX-$XMIN)/15, $YMIN + 100 ,10, 0, 1, 10, "v",$PS_file);
#        &PSTEXT($XMAX + 30, $YMIN - 150 ,11, 0, 1, 10, "Distance (km)",$PS_file);
#        &PSTEXT($XMIN - 300, $YMIN +($YMAX - $YMIN)/2,12, 90, 1, 10, "Thickness (km)",$PS_file);
        &PSTEXT($XMIN + 0, $YMAX + 50 ,16, 0, 1, 10, $TITLE,$PS_file);
        &PSTEXT($XMIN + 450, $YMAX + 50 ,12, 0, 1, 10, $m_time,$PS_file);
   }
   else 
   {  
       $yshift2 = -($YLEN + 0.56);
       $BSCALE = "-B${xanot}/${yanot}s";
       `psbasemap -JX$SCALE -R$RSCALE -K -O $PORT  -X0 -Y$yshift2 $BSCALE>>$PS_file`;
        &PSTEXT($XMIN+($XMAX-$XMIN)/15, $YMIN + 100 ,10, 0, 1, 10, "r",$PS_file);
   }

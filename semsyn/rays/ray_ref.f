...
c     for the SKS phases
c     SV=3  SH=4 P=5
c   Modifying history
c   Oct 12, 2006: add reflection phases
CCC*******************************
CCC*******************************
...
c  depth reflection phases
          iref = isd - 1 + isd
	  iseg1=ilayd-isd
	  iseg2=ilayd-itd+1
	  itotal=iref + iseg1+iseg2+1 
	  write(*,*)ip,' ilayd =',ilayd,'  itotal =',itotal
	  do j=isd-1,1, -1
	    na(isd - j)=j
            nray(isd-j)=MODSV
	  ...
	  enddo
	  do j=1,iseg2
	    na(j+iref+iseg1)=ilayd-j+1
            if(ilayd-j+1.ge.nlaySK) then
  	        nray(j+iref+iseg1)=MODP
            else
	        nray(j+iref+iseg1)=MODSV
            endif
	  enddo
	  na(itotal)=1
	  nray(itotal)=MODSV

...
$deg = 6371*$pi/180.;
$boundary = 104.8;
$top = 446.27; # based upon semmodel after flatterning
...
#   $rawdir = "/home/zhl/work/aniso/data/velocity/new_v/1998.361.00/";
   $rawdir = "/home/zhl/work/aniso/data/new/1998.361.00/";
   chdir $rawdir;
...
if($draw_model == 1) {
   ...
   $XLEN   = 12;
   $YLEN   = -$XLEN/($XMAX-$XMIN)*($YMAX-$YMIN)/$deg/2;
   $Line = "4/50/50/50";
   ...
   `surface $velm -Gvelm.grd -R$RSCALE -I0.1/0.1 -T0.1 -V`;
   `grdimage velm.grd -X$xshift -Y$yshift -JX$SCALE -R$RSCALE -C$CPT -P -K -O >> $PS_file`;
   ...
   &PSTEXT($XMIN+($XMAX-$XMIN)/2.3 ,$YMAX + ($YMAX-$YMIN)/5,11, 0, 1, 10, "Distance (\260)",$PS_file);
   &PSTEXT($XMIN - 0.5, $YMIN -($YMAX - $YMIN)/20,16, 0, 1, 10, "(a)",$PS_file);
...
$plot_sc_error = 1;
$resfile1 = "split_par.txt";
$resfile2 = "split_par002_02.txt";

if($plot_sws){
   print "Plotting split parameters\n";
   &plot_split($resfile1,$resfile2);
}

`gs sks.ps`;

sub plot_split{
   my($res_file,$res_file2) = @_;
...
# ------------- phi: fast polarization -------------------------------------
   ...
   &PSTEXT($XMIN-($XMAX-$XMIN)/4.2,$YMAX +($YMAX - $YMIN)/8 ,16, 0, 1, 10, "(c)",$PS_file);

   $Line = "3/30/30/30";
   &read_sws($res_file,1);
   `psxy line.xy -JX$SCALE -M -R$RSCALE  -N -K -O $PORT -W$Line  >> $PS_file`;
   `psxy dot.xy  -JX$SCALE  -R$RSCALE  -N -K -O $PORT -W$Line -Sc0.15  >> $PS_file`;

   $Line = "3/30/30/230";
   &read_sws($res_file2,1);
   `psxy line.xy -JX$SCALE -M -R$RSCALE  -N -K -O $PORT -W$Line  >> $PS_file`;
   `psxy dot.xy  -JX$SCALE  -R$RSCALE  -N -K -O $PORT -W$Line -Sc0.15  >> $PS_file`;
# -------------- dt: delay time   -----------------------------------------
      $YMIN = 0;  $YMAX = 3.2;
...
sub read_sws{
   my($file,$degree) = @_;
   open(FF,"$file");
   print "dealing with $file degree=$degree\n";
   open(LL,">line.xy");
   open(DD,">dot.xy");
   while($line = <FF>){
      chomp($line);

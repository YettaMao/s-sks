...
int k;
{
    ...
    if(i==0 && k ==0) value = -Na*(Na+1)/4.;
        ...
        rk = gll[k]; ri= gll[i];
        value = Legendre(Na,rk)/Legendre(Na,ri)/(rk-ri);
    }
    return value;
}
...
-------------------
  ...
  1	x							1
  2     1/2*(3x**2-1)						3x
  3     1/2*(5x**3-3*x)						1/2*(15x**2-3)
  4     1/8*(35x**4-30x**2+3)					1/8*(140x**3-60x)
  5     1/8*(63x**5-70x**3+15x)					1/8*(315x**4-210x**2+15)
  6     1/16*(231x**6-315x**4+105x**2-5)			1/16*(1386x**5-1260x**3+105x)
  7     1/16*(429x**7-693x**5+315x**3-35x)
  8	1/128*(6345x**8-12012x**6+6930x**4-1260x**2+35)
  9     1/128*(12155x**9-25740x**7+18018x**5-4620x**3+315x)
-------------------
*/
float Legendre2(Nl,x)
int Nl;
float x;
{
    float pn;
    if(Nl>8) printf("Error invoke: degree is >8 !, in Legendre2() \n");
    if(Nl==0)	pn= 1.;
    if(Nl==1)	pn= x;
    if(Nl==2)	pn= 1./2.*  (3.   *x*x             -1.);
    if(Nl==3)	pn= 1./2.*  (5.   *x*x*x           -3.*x);
    if(Nl==4)	pn= 1./8.*  (35.  *x*x*x*x         -30.*x*x+ 3.);
    if(Nl==5)	pn= 1./8.*  (63.  *x*x*x*x*x       -70.*x*x*x+ 15.*x);
    if(Nl==6)	pn= 1./16.* (231. *x*x*x*x*x*x     -315.*x*x*x*x+ 105.*x*x - 5.);
    if(Nl==7)	pn= 1./16.* (429. *x*x*x*x*x*x*x   -693.*x*x*x*x*x+ 315.*x*x*x - 35.*x);
    if(Nl==8)	pn= 1./128.*(6345.*x*x*x*x*x*x*x*x -12012.*x*x*x*x*x*x+ 6930.*x*x*x*x - 1260.*x*x + 35.);
    return pn;
}
...
float x;
{
    ...
    if(Nl == 0) {dpn = 0;}
       ...
       dp1= 1; dpn1 = dp1;
       for(i=2; i<=Nl;i++){
/*071409, LZ tested that Legendre() is the same as Legendre2() for hpn = 5*/
          pn1 = Legendre(i-1,x);
//          pn1 = Legendre2(i-1,x);
...
/*
  Last Modified: 062809, LZhao, 201 McCone Hall
  References: Wiki Pedia: Legendre polynomias
-------------------
  n	Pn(x)							Pn'(x)
  0	1							0
  ...
  5     1/8*(63x**5-70x**3+15x)					1/8*(315x**4-210x**2+15)
  6     1/16*(231x**6-315x**4+105x**2-5)			1/16*(1386x**5-1260x**3+105x)
  7     1/16*(429x**7-693x**5+315x**3-35x)                      1/16*(3003x**6-3465x**4+945x**2-35)
  8	1/128*(6345x**8-12012x**6+6930x**4-1260x**2+35)
  9     1/128*(12155x**9-25740x**7+18018x**5-4620x**3+315x)
-------------------
  np: degree of Polynomials,
...
{
    if(np > 6 || np<3) {
       printf("Error invoke: degree is out of 3,4, 5 or 6!, in GLL() \n");
       exit(-1);
    }
    ...
    if(np==6){
        ...
        gllp[3] = 0.;
        gllp[4] =  sqrt((-c - sqrt(c*c-4*a*e))/2/a);
        gllp[5] =  sqrt((-c + sqrt(c*c-4*a*e))/2/a);
        gllp[6] = 1.;
    }
}

...
/* GRT-SEM interface description*/
   ...
   long int rec_len;
   float **sy, **matrix(); /*For GRT-SEM interfaces*/
   int * ns;
   float rdt; /* = dst/dp */
   void interpl();
/* source definition*/
...
/* subroutine definition, all subroutine need to be declare here before use*/
   ...
   void  atten_taper(), atten_wave(); /*attenuation*/
...
// To avoid singularity in GRT resolution
                for(gl=0;gl<mtotal;gl++){
                    if(!(grtuw[gl]<10 && grtuw[gl]>-10)) {
                        // printf("gl=%d grtuw=%e\n",gl,grtuw[gl]);
                        if(gl==0) grtuw[gl] =0; 
                        else grtuw[gl] = (grtuw[gl-1]+grtuw[gl+1])/2;
                    }
                }
            }
      }
      ...
      if(it==0) { 
          Initial_uvw(uvw);
          Initial_uvw(uvw0);
          if(msource == 0)  GRT_Interface(uvw,grtuw);
      }
      else{
...
//          if(msource == 0) { if(it < nt) GRT_Interface3(uvw,uvw0,grtuw,grtuw0);}

          Stiff_Matrix(GStiff,uvw,meshs,uniform);

          if(msource == 1) Time_diff(it,uvw1,uvw,uvw0,GStiff,Mass,meshs,sdt,msource,mex,mez,Force,absorb,Ab_matrix); 
          else   Time_diff_hybrid(it,uvw1,uvw,uvw0,GStiff,Mass,meshs,sdt,absorb,Ab_matrix);
...
//   struct cmatrix2d apoint;
   ...
   len = (long) (nx * nz+1) * sizeof(struct cmatrix);
   ...
   if(readpars == 1){
        ...
        for(j=0; j<nll; j++){
            ...
            for(i=0; i<np[j]; i++){
                fscanf(fop,"%f ",&f0);
                bn[j][i] = f0;
            /*20140101: add flatten approximation; 20140105, flattern it in constructing model, comment it again*/
/*                if(flat == 0){ 
                    f0 = thickness - f0;
                    f0 = 6371/(6371-f0);
                    bn[j][i] *= f0;
                    if(i==0) printf("layer No.%d bn=%-8.3f -> bn=%-8.3f\n",j,bn[j][i]/f0,bn[j][i]);
                }
*/
            } 
            fscanf(fop,"\n");
        ...
        for(i=0; i< nx; i++){  
            ...
            for(j=firstlayer; j<nll; j++){
                ...
                f0 = (iz1+iz2)/2.*h;
                if(flat ==0) f0 = 6371/(6371-f0);
                else f0 = 1.0; 
                f0 = 1.; /*20130106: addede this line because I have flatted the model in input model file first*/
	        if(iz2 > nz-1) iz2 = nz-1;
//		if(!(i % 100)) printf("x=%f, j = %d, iz1 = %d,  iz2 = %d\n",x,j,iz1,iz2);
                ...
                if(ratio[j] == 1){ /*if assign by multiplying with coefficients*/
                    ...
                    apoint.c11 = f0*t_c11;   apoint.c12 = f0*mc12[j]; apoint.c13 = f0*mc13[j]; apoint.c16 = f0*mc16[j];
                    apoint.c22 = f0*mc22[j]; apoint.c23 = f0*mc23[j]; apoint.c26 = f0*mc26[j]; apoint.c33 = f0*mc33[j]; apoint.c36 = f0*mc36[j];  
                    apoint.c44 = f0*t_c44;   apoint.c45 = f0*mc45[j]; apoint.c55 = f0*mc55[j]; apoint.c66 = f0*mc66[j];
                    apoint.rho = f0*rho[j];
                }
                for(i1=0;i1<3;i1++){
                    for(j1=0;j1<3;j1++){
...
*/
void Time_diff(it,p1,p,p0,GStiff,Mass,meshs,sdt,msource,mex,mez,Force,absorb,ab_matrix)
int    it;
   ...
   if(msource == 1){
      ...
      if(absorb==1){
          for(i=0;i<index;i++){
             coe = -dt2/Mass[i];
             for(k=0;k<3;k++){
                p1[i].u[k] = coe*GStiff[i].u[k] + 2.*p[i].u[k] - p0[i].u[k];
             }
          }
          for(k=0;k<3;k++){
                tempf = Force.u[k] - GStiff[index].u[k];
                p1[index].u[k] = dt2*tempf/Mass[index] + 2.*p[index].u[k] - p0[index].u[k];
          }
...
Require:
  ...
  absorb: 1: using absorbing boundary condition
         note that ab_matrix has multiplied sdt/2
*/
void Time_diff_hybrid(it,p1,p,p0,GStiff,Mass,meshs,sdt,absorb,ab_matrix)
int    it;
struct vector * p1,*p,*p0;
struct vector * GStiff;
float *Mass, sdt;
...
//   printf("4.3 G=%e ukw=%e uvw1=%e\n",GStiff[hpn*nx + 5*hpn].u[2],p[hpn*nx + 5*hpn ].u[2],p1[hpn*nx + 5*hpn].u[2]);
      ...
      for(j=0;j<nez;j++){
         for(m=0;m<=hpn;m++){
            ...
            for(n=0;n<=hpn;n++){
                ...
                /*052511, tried the following equation, almost the same*/
//                mass[index] = rx * rz * je * density;
//                printf("%d %d %f %f je=%f den=%f mass[%d]= %f\n", m,n,rx,rz,je, density, index, mass[index]);
            }
         }
      }
   }
...
/* 20110610: tried using the following equation which gave the same results   
   if(i == 1 && j == 1) stress = 0.;
   else{
    ...
    item = 0.; 
       ...
       if(i==0) {
          for(theta=0;theta<=hpn;theta++){
              ...
              item  = item + p[index].u[j] * dlagP[alpha*(hpn+1)+theta];
//if(ex==5 &&ez==0&&alpha==0&&beta==0) printf("u=%e ",p[index].u[j]);
          }
//if(ex==5 &&ez==0&&alpha==0&&beta==0) printf("\n");
          uji = item * Uni_JeI;
       }
       ...
       if(i==2) {
          for(theta=0;theta<=hpn;theta++){
              ...
              item  = item + p[index].u[j] * dlagP[beta*(hpn+1)+theta];
//if(ex==0 &&ez==16&&alpha==0&&beta==0) printf("u=%e ",p[index].u[j]);
          }
//if(ex==0 &&ez==16&&alpha==0&&beta==0) printf("ukw=%e\n", p[(ez*hpn+hpn)*nx + ex*hpn + alpha].u[j]);
          uji = item * Uni_JeI;
       }
    return uji;
}
...
int uniform; /*Partition uniformly or not*/
{
    ...
    for(theta=0;theta<=hpn;theta++){
         index = (ez*hpn+beta)*nx + ex*hpn + theta;
         duji[0][0]  = duji[0][0] + p[index].u[0] * dlagP[alpha*(hpn+1)+theta];
         duji[1][0]  = duji[1][0] + p[index].u[1] * dlagP[alpha*(hpn+1)+theta];
         duji[2][0]  = duji[2][0] + p[index].u[2] * dlagP[alpha*(hpn+1)+theta];
    }
    /*partial derivative wrt z: i=2*/
    for(theta=0;theta<=hpn;theta++){
         index = (ez*hpn+theta)*nx + ex*hpn + alpha;
         duji[0][2]  = duji[0][2] + p[index].u[0] * dlagP[beta*(hpn+1)+theta];
         duji[1][2]  = duji[1][2] + p[index].u[1] * dlagP[beta*(hpn+1)+theta];
         duji[2][2]  = duji[2][2] + p[index].u[2] * dlagP[beta*(hpn+1)+theta];
    }
    for(j=0;j<3;j++){
...
//if(ix==20) printf("iz=%d nz=%d ind=%d\n",iz,nz,nh-iz-1);
               offset = iz*nx+ix;
...
//if(iz==20) printf("ix=%d nx=%d ind=%d\n",ix,nz,nh-ix-1);
               offset = iz*nx + ix;
              for(i=0;i<3;i++) p[offset].u[i] *= ptaper[0];
         }
...
void GRT_Interface2(p,grtuw,grtuw0)
struct vector * p;
float *grtuw, *grtuw0;
{
   int i,m;
...
/* method 2: adding scattering energy */
/*   for(i=nbl;i<nx;i++){
      p[i].u[0] += - grtuw0[i]    + grtuw[i];
      p[i].u[2] += + grtuw0[i+nx] - grtuw[i+nx];
   }
   for(i=nbl;i<nz;i++){
      index= i*nx+nbl;
...
*/
/* method 3: minus scattering */
/*   for(i=nbl;i<nx;i++){
      p[i].u[0] = - (p[i].u[0] - grtuw0[i])    + grtuw[i];
      p[i].u[2] = - (p[i].u[2] + grtuw0[i+nx]) - grtuw[i+nx];
   }
   for(i=nbl;i<nz;i++){
      index= i*nx+nbl;
      p[index].u[0] = -(p[index].u[0] - grtuw0[2*nx+i]) + grtuw[2*nx+i];
      p[index].u[2] = -(p[index].u[2]+ grtuw0[2*nx+nz+i]) - grtuw[2*nx+nz+i];
   }
*/
}


void GRT_Interface3(p,p0,grtuw,grtuw0)
struct vector * p,*p0;
float *grtuw, *grtuw0;
{
   int i,m;
...
/*
   for(i=istart;i<nx;i++){
      p[i].u[0] =  p0[i].u[0] + grtuw[i];
      p[i].u[2] =  p0[i].u[2] - grtuw[i+nx];
   }
   istart = neh * hpn + 1;
   for(i=istart;i<nz;i++){
      index= i*nx + nbl;

...
/*                                                                */
/*                                                                */
/* Last modified: July. 1, 2010 by L Zhao                         */
...
/* 062710 LZ: output: velocity-> displacement, new method         */
/*            by conv with source, refer to uvw2sac.c in ../plot  */
/******************************************************************/
...
int     lay670 = 37;     /* the layer number for the 670  (PREM) */
...
/* source: GRT output green funtion with displacement component*/
   int displace,key,idev;
   float amom,dt1, dt2, dt3, rhos,zs;
...
/* following for the ray descriptions */
    ...
    void conv_source();/*convolve with source parameters*/
...
/* Grid point */
    ...
    if(plot_trace == 1){
        npts = (int) (nt/kdt);
        ixbeg = (int) (xbeg/h);
        fopu = open_file("grt.u","wt");
        fprintf(fopu,">> %d %d\n", xtrace,npts);
        fopw = open_file("grt.w","wt");
        fprintf(fopw,">> %d %d\n", xtrace,npts);
...
/* Calculate the initial responses at n=3 and 4 */
    ...
    printf("nx= %d nd= %d nz= %d\n",nx,nd,nz);
    int kk,is_nan; /* zhl, 6/16/2006 */
    float rdep;/*zhl, 062810, receiver depth */
    for(n34=3; n34<=4; n34++){ 
	dep = 1.e-4;
        rdep = nz*h;
...
/*LZ: change the output to be displacement instead of velocity, 062810, 430 IGGCAS */
                if(displace==1) conv_source(green,nt,dp, 0.,zs,xx,rdep,key,amom,dt1,dt2,dt3,idev);
...
/* zhl for debug, find nan output, 6/16/2006 */
                ...
                if(is_a_rec ==1 && plot_trace ==1){
                    if(n34 == 3){
                       ...
                       fprintf(fopw,">> %f\n", xx);
                       for(kk=0; kk < nt; kk ++){
                          if(!(kk%kdt))  fprintf(fopw,"%-9.3f %-12.6e\n",kk*dp,gcarc+green[kk]/ampmax*gain);
                       }            
                    }                    
                }
...
//                printf("DIRECT WAVE\n");
		...
		k  = MultiBounces(nrec,nfil,zlay,k,nbounce,type,
                ...
                n++;
		if(idirect > 0) {
//                printf("converted WAVE at 671, 391, 211, moho and middle crust\n");
	        for(i=4; i<=5; i++){
                   nenn=0;
		   ...
		   if(i==1) layspecial = lay670;
		   if(i==2) layspecial = lay390;
		   if(i==3) layspecial = lay210;
		   if(i==4) layspecial = laycrust;
		   if(i==5) layspecial = layincrust;
		   k  = MultiBounces(nrec,nfil,zlay,k,nbounce,type,
				   na0,nray0,&ncoun0[n],&nen0[n],layspecial);
                   n++;
		}
...
/* subroutine to convolve with source parameters
num: time step number
value storing time sequence of velocity at the beginning
-> storing time sequence of displacement 
dt: time step
xs,zs,xr,zr: positions of the source and receiver
key: switch for point souce 1: point source
amom,dt1,dt2,dt3,idev: source parameters
*/
void conv_source(value,num,dt,xs,zs,xr,zr,key,amom,dt1,dt2,dt3,idev)
float * value;
int num,idev;
float dt,xs,zs,xr,zr,dt1,dt2,dt3;
int key;
{
/* call source.f here*/
...
/* modify with 1/sqrt(r) */
    ...
    for(k=0;k<num;k++){
       nnn = (int) k/2;
       value[k] = b[nnn]+(b[nnn+1]-b[nnn])/sdt*(k*dt-nnn*sdt);
    } 
    free(b); free (pp);
/* interpolate value*/
}

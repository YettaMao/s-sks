   ...
   if(check == 1){
      ...
      for(i=0;i<nx;i++){
          for(j=0;j<nz;j++){
              ...
              fprintf(fom,"%-8.1f %-8.1f %-8.2f\n", (xmin+i*h)/111.2,(nz-1-j)*h,tempv);
          }
      }
      fclose(fom);
   }
  ...
  for (ix=0; ix<nx; ix++)
      for (iz=0; iz< nz; iz++){ 
           ...
           if(atten>1 && ix< nh0){
               ptaper = taper + nh0 - ix -1;
               offset = iz*nx + ix;
               if(iz>=nh0) for(i=0;i<3;i++) p[offset].u[i] *= ptaper[0];
         }
           ...
           if(atten>3 && iz>= nz-nh0){
               ptaper = taper + iz - nz + nh0;
               offset = iz*nx+ix;
               if(ix<nx-nh0 && ix>=nh0) for(i=0;i<3;i++) p[offset].u[i] *= ptaper[0];
           }
       }
}
/*
